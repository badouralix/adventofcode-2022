...
var a: std.mem.Allocator = if (!builtin.is_test) undefined else std.testing.allocator;
...
fn run(input: [:0]const u8) [:0]const u8 {
    ...
    while (line_it.next()) |line| {
        ...
        const amount = std.fmt.parseInt(u16, parts_it.next().?, 10) catch unreachable;
        ...
        var buf = [_]u8{0} ** 50; // buffer to store popped chars
        var amount_it: usize = 0;
        while (amount_it != amount) : (amount_it += 1) {
            buf[amount_it] = queues[source - 1].pop().?;
        }
        while (amount_it != 0) : (amount_it -= 1) { // Start from amount_it, so we don't have to check for nullchar
            queues[destination - 1].prepend(buf[amount_it - 1]) catch unreachable;
        }
    }
...
test "ez" {
    ...
    try stdout.print("\n", .{});
    const ans = run(input);
    try std.testing.expect(std.mem.eql(u8, ans, "MCD"));
}
