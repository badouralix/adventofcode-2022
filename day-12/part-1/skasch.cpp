...
class Map {
 public:
  ...
  int FindShortestPath() {
    std::priority_queue<BFSState, std::vector<BFSState>,
                        std::function<bool(const BFSState&, const BFSState&)>>
        deque([&](const BFSState& lhs, const BFSState& rhs) {
          return Distance(lhs) > Distance(rhs);
        });
    deque.emplace(end_, 0);
    std::set<Pos> visited;
    BFSState state;
    while (!deque.empty()) {
      state = deque.top();
      ...
      for (const Pos& neighbor :
        ...
        deque.emplace(neighbor, state.steps + 1);
      }
    }
 ...
 private:
  ...
  int Distance(const BFSState& state) const {
    return state.steps + std::abs(state.pos.first - start_.first) +
           std::abs(state.pos.second - start_.second);
  }
};

std::string Run(const std::string& input) {
  Map map(input);
  return std::to_string(map.FindShortestPath());
}
