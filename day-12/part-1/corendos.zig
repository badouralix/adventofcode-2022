...
fn run(input: [:0]const u8) u64 {
    ...
    costs.getAtPtr(state.start.x, state.start.y).* = 0;
    var visited = Grid(bool).init(dim, false, a);
    var in_frontier = Grid(bool).init(dim, false, a);
    in_frontier.getAtPtr(state.start.x, state.start.y).* = true;

    var frontier = PriorityQueue.init(a, &costs);
    frontier.add(state.start) catch unreachable;

    while (true) {
        if (frontier.count() == 0) @panic("Uh oh");

        var node = frontier.remove();
        if (node.x == state.end.x and node.y == state.end.y) {
            return costs.getAt(node.x, node.y);
        }
        ...
        if (node.x > 0) {
            ...
            if (neighbor_height - 1 <= node_height) {
