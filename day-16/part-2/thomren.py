...
class ThomrenSubmission(SubmissionPy):
    def run(self, s):
        ...
        def solve(
            minutes,
            start="AA",
            remaining_valves=frozenset(flows.keys()),
            elephant=False,
        ):
            res = max(
                # the elve continues to open more valves
                [
                    ...
                    if dist[start, valve] < minutes
                ]
                # the elephant handles the remaining valves
                + (
                    [solve(26, remaining_valves=remaining_valves, elephant=False)]
                    if elephant
                    else []
                )
                # no more elephant and no more valves to open
                + [0]
            )
            return res

        return solve(26, elephant=True)
