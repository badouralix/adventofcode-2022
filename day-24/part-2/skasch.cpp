...
void Move(const Pos& pos, const Map& map,
  ...
  if (Row(pos) == map.n_inner_rows + 1) {
    callback(pos);
    Pos delta = kDirectionDeltas.at('^');
    if (!map.blizzards.contains(pos + delta)) {
      callback(pos + delta);
    }
    return;
...
std::string Run(const std::string& input) {
  ...
  int result = 0;
  result += Bfs(parsed_input.map, parsed_input.start,
                parsed_input.end + kDirectionDeltas.at('^'));
  result += Bfs(parsed_input.map, parsed_input.end,
                parsed_input.start + kDirectionDeltas.at('v'));
  result += Bfs(parsed_input.map, parsed_input.start,
                parsed_input.end + kDirectionDeltas.at('^'));
