...
fn run(input: &str, y: isize) -> usize {
    ...
    (xmin..=xmax)
        .filter(|&x| {
            sensors
                .iter()
                .any(|s| manhattan_dist((s.x, s.y), (x, y)) <= s.range())
        })
        .filter(|&x| !(sensors.iter().any(|s| (s.x, s.y) == (x, y))))
        .filter(|&x| !(sensors.iter().any(|s| s.closest_beacon == (x, y))))
        .count()
}
