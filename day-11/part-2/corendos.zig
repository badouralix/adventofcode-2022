    ...
    pub fn runRound(self: *State) void {
        for (self.monkeys) |*monkey| {
            while (monkey.popItem()) |item| {
                ...
                // NOTE(Corendos): gut feeling, it works who the fuck knows why (spoiler: math surely knows)
                worry_level = @mod(worry_level, self.divider);
...
pub fn parseInitialState(input: []const u8) State {
    ...
    while (monkey_details_it.next()) |monkey_details| {
        ...
        // Parse test value
        {
            ...
            divider *= current_monkey.test_value;
        }
    ...
    return State{ .monkeys = monkey_list.toOwnedSlice(), .divider = divider};
}
