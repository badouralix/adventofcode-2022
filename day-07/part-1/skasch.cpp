...
constexpr inline int kMaxFolderSize = 100000;
...
class FileSystem {
 public:
  FileSystem() : nodes_{{-1, {}}}, data_{Folder{"/"}}, position_{0} {}
...
int DFSFolderSize(const FileSystem& file_system, int position,
                  int max_folder_size, int& valid_folders_total_size) {
  const Data& data = file_system.GetData(position);
  switch (data.index()) {
    case 0: {  // Folder
      int folder_size = 0;
      for (const auto& [_, child_id] : file_system.GetNode(position).children) {
        folder_size += DFSFolderSize(file_system, child_id, max_folder_size,
                                     valid_folders_total_size);
      }
      if (folder_size <= max_folder_size) {
        valid_folders_total_size += folder_size;
      }
      return folder_size;
    }
    case 1:  // File
...
std::string Run(const std::string& input) {
  ...
  int valid_folders_total_size = 0;
  DFSFolderSize(file_system, 0, kMaxFolderSize, valid_folders_total_size);
  return std::to_string(valid_folders_total_size);
}
