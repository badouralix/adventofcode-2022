...
fn run(input: &str) -> u32 {
    ...
    fn visit_sum_size(total: &mut u32, tree: &Tree<FsNode>, node: Handle) -> u32 {
        let mut sum = 0;
        for_children!(c of node node in graph tree {
            match &tree.get(c).unwrap().content {
                FsNode::Dir { .. } => {
                    let dir_size = visit_sum_size(total, tree, c);
                    sum += dir_size;
                },
    ...
    let mut total = 0;
    visit_sum_size(&mut total, &tree, tree.root.unwrap());
    total
}
