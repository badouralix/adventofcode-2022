...
class DidipSubmission(SubmissionPy):
    def run(self, s):
        ...
        root = tree_weights['/']
        to_delete = 30000000 - 70000000 + root
        return min(filter(lambda x: x > to_delete, tree_weights.values()))

def test_didip():
    """
    Run `python -m pytest ./day-07/part-2/didip.py` to test the submission.
    """
    assert (
        DidipSubmission().run(
            """$ cd /
